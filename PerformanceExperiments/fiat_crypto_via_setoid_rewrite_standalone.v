...
Strategy 100 [Let_In].
Reserved Notation "'tdlet' x .. y := v 'in' f"
         (at level 200, x binder, y binder, f at level 200, format "'tdlet'  x .. y  :=  v  'in' '//' f").
Definition tLet_In {A P} (x : A) (f : forall a : A, P a) : P x := let y := x in f y.
Notation "'tdlet' x .. y := v 'in' f" := (tLet_In v (fun x => .. (fun y => f) .. )).
Strategy 100 [tLet_In].
...
Global Instance Proper_tLet_In_nd_changebody {A P R} {Reflexive_R:@Reflexive P R}
  : Proper (eq ==> pointwise_relation _ R ==> R) (@tLet_In A (fun _ => P)).
Proof. lazy; intros; subst; auto; congruence. Qed.

Global Instance Proper_tLet_In_nd_changevalue {A B} (RA:relation A) {RB:relation B}
  : Proper (RA ==> (RA ==> RB) ==> RB) (tLet_In (P:=fun _=>B)).
Proof. cbv; intuition. Qed.

Lemma Proper_tLet_In_nd_changebody_eq {A P R} {Reflexive_R:@Reflexive P R} {x}
  : Proper ((fun f g => forall a, x = a -> R (f a) (g a)) ==> R) (@tLet_In A (fun _ => P) x).
Proof. lazy; intros; subst; auto; congruence. Qed.

Global Instance Proper_tLet_In_nd_changevalue_forall {A B} {RB:relation B}
  : Proper (eq ==> (forall_relation (fun _ => RB)) ==> RB) (tLet_In (P:=fun _:A=>B)).
Proof. cbv; intuition (subst; eauto). Qed.

(* Strangely needed in some cases where we have [(fun _ => foo) ...] messing up dependency calculation *)
Hint Extern 1 (Proper _ (@tLet_In _ _)) => progress cbv beta : typeclass_instances.
...
Lemma unfold_repeat_reduce (n : nat) (s:Z) (c:list _) (p:list _)
        ...
        (fun n' repeat_reduce_n' p
         => tdlet p := p in
              ...
              (prod_rect
                 (fun _ => _)
                 (fun hi lo
                  => (lo, map (fun t => (fst t / s, snd t)) hi))
                 (partition (fun t => fst t mod s =? 0) p)))
        n
        p.
Proof.
  cbv [Associational.repeat_reduce Associational.split tLet_In].
  revert p; induction n as [|n IHn]; cbn; [ reflexivity | ].
  intro; rewrite IHn.
  edestruct partition; reflexivity.
Qed.
Hint Rewrite unfold_repeat_reduce : mydb.
...
Definition insert_idA_listZ x : x = @idA (list Z) x := @insert_idA _ _.

Hint Rewrite Z_of_nat_O Z_of_nat_S Z_mul_pos_pos Z.mul_0_l Z.mul_0_r Z.opp_0 Z_div_0_l_pos Z_opp_pos Z_opp_neg unfold_Z_div_pos_pos unfold_Z_div_pos_neg unfold_Z_div_neg_pos unfold_Z_div_neg_neg Z.pow_0_r Z_pow_pos_pos Z_modulo_pos_pos Z_eqb_pos_pos Z.eqb_refl Nat.eqb_refl Z_eqb_neg_neg Z_eqb_pos_0 Z_eqb_0_pos Z_eqb_pos_neg Z_eqb_neg_pos Z_eqb_neg_0 Z_eqb_0_neg length_nil nat_eqb_S_O nat_eqb_O_S : myearlydb.
Hint Rewrite <- pred_Sn : myearlydb.
...
Goal True.
  ...
  rewrite_strat
    (repeat
       ((subterm insert_idA_listZ; subterm remove_idA);
        (eval mycbv; eval cbn [List.app fst snd nat_rect]);
        (choice
           (progress
              (topdown
                 (repeat
                    (choice
                       (choice
                          (hints letdb)
                          (progress (eval mycbv; eval cbn [List.app fst snd])))
                       (choice
                          (hints myearlydb)
                          (hints mydb)))
           )))
           (progress eval cbv [Positional.from_associational
                                 Positional.place
                                 Positional.zeros repeat
                                 Positional.add_to_nth
                                 Positional.chained_carries])
    ))).
  cbv [Positional.carry_reduce]. Positional.carry Positional.to_associational seq Associational.carry Associational.carryterm].
  Print Positional.carry_reduce.
  Print Positional.chained_carries.
  Print Associational.repeat_reduce.
                  | progress cbv [Associational.repeat_reduce]
                | progress cbv [Positional.from_associational]
                | progress cbv [Positional.zeros repeat]
                | progress cbv [Positional.place]
